...
const routingStore = new db_1.JsonStore("process-routings");
const operationStore = new db_1.JsonStore("process-operations");
const equipmentStore = new db_1.JsonStore("equipment");
// ===== Process Routings =====
...
router.patch("/routings/:id", async (req, res) => {
    const { id } = req.params;
    const update = {};
    if (req.body.routingNumber !== undefined)
        update.routingNumber = req.body.routingNumber;
    if (req.body.partNumber !== undefined)
        update.partNumber = req.body.partNumber;
    if (req.body.partName !== undefined)
        update.partName = req.body.partName;
    if (req.body.partRevision !== undefined)
        update.partRevision = req.body.partRevision;
...
router.delete("/routings/:id", async (req, res) => {
    ...
    for (const op of operations)
        await operationStore.delete(op.id);
...
router.patch("/operations/:opId", async (req, res) => {
    ...
    if (req.body.operationName !== undefined)
        update.operationName = req.body.operationName;
    if (req.body.operationDescription !== undefined)
        update.operationDescription = req.body.operationDescription;
    if (req.body.department !== undefined)
        update.department = req.body.department;
    if (req.body.workCenter !== undefined)
        update.workCenter = req.body.workCenter;
    if (req.body.equipmentCode !== undefined)
        update.equipmentCode = req.body.equipmentCode;
...
router.patch("/equipment/:id", async (req, res) => {
    ...
    if (req.body.equipmentName !== undefined)
        update.equipmentName = req.body.equipmentName;
    if (req.body.equipmentType !== undefined)
        update.equipmentType = req.body.equipmentType;
