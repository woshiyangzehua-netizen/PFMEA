...
import type { Pfmea, PfmeaStructureNode, PfmeaFunction, PfmeaFailure, PfmeaRiskAnalysis } from "../db";
import { calculateAP } from "../utils/ap-calculator";

const router = Router();

const pfmeaStore = new JsonStore<Pfmea>("pfmea");
const structureStore = new JsonStore<PfmeaStructureNode>("pfmea-structure-nodes");
const functionStore = new JsonStore<PfmeaFunction>("pfmea-functions");
const failureStore = new JsonStore<PfmeaFailure>("pfmea-failures");
const riskStore = new JsonStore<PfmeaRiskAnalysis>("pfmea-risk-analyses");
...
router.patch("/:id", async (req, res) => {
  ...
  if (req.body.productName !== undefined) update.productName = req.body.productName;
  if (req.body.partNumber !== undefined) update.partNumber = req.body.partNumber;
  if (req.body.analysisBoundary !== undefined) update.analysisBoundary = req.body.analysisBoundary;
  if (req.body.analysisTeam !== undefined) update.analysisTeam = JSON.stringify(req.body.analysisTeam);
  if (req.body.currentStep !== undefined) update.currentStep = req.body.currentStep;
...
router.delete("/:id", async (req, res) => {
  ...
  // Cascade delete
  const risks = await riskStore.findMany((r) => r.pfmeaId === id);
  for (const r of risks) await riskStore.delete(r.id);
  const failures = await failureStore.findMany((f) => f.pfmeaId === id);
  for (const f of failures) await failureStore.delete(f.id);
  const functions = await functionStore.findMany((f) => f.pfmeaId === id);
  for (const f of functions) await functionStore.delete(f.id);
  const structures = await structureStore.findMany((s) => s.pfmeaId === id);
  for (const s of structures) await structureStore.delete(s.id);
...
router.patch("/:id/structures/:nodeId", async (req, res) => {
  ...
  const update: Partial<PfmeaStructureNode> = {};
  ...
  if (req.body.focusElement !== undefined) update.focusElement = req.body.focusElement;
...
router.patch("/:id/risks/:riskId", async (req, res) => {
  ...
  const update: Partial<PfmeaRiskAnalysis> = {};

  if (req.body.occurrence !== undefined) update.occurrence = Number(req.body.occurrence);
  if (req.body.detection !== undefined) update.detection = Number(req.body.detection);
  if (req.body.severity !== undefined) update.severity = Number(req.body.severity);
  if (req.body.currentPreventionMeasures !== undefined) update.currentPreventionMeasures = JSON.stringify(req.body.currentPreventionMeasures);
  if (req.body.currentDetectionMeasures !== undefined) update.currentDetectionMeasures = JSON.stringify(req.body.currentDetectionMeasures);
  if (req.body.recommendedActions !== undefined) update.recommendedActions = JSON.stringify(req.body.recommendedActions);
  if (req.body.responsiblePerson !== undefined) update.responsiblePerson = req.body.responsiblePerson;
  if (req.body.targetDate !== undefined) update.targetDate = req.body.targetDate;
  if (req.body.actionStatus !== undefined) update.actionStatus = req.body.actionStatus;
  if (req.body.optimizedOccurrence !== undefined) update.optimizedOccurrence = Number(req.body.optimizedOccurrence);
  if (req.body.optimizedDetection !== undefined) update.optimizedDetection = Number(req.body.optimizedDetection);
  if (req.body.optimizedSeverity !== undefined) update.optimizedSeverity = Number(req.body.optimizedSeverity);
  ...
  if (existing) {
    ...
    update.actionPriority = calculateAP(sev, occ, det);
    ...
    if (update.optimizedOccurrence || update.optimizedDetection || update.optimizedSeverity) {
      ...
      update.optimizedAp = calculateAP(os, oo, od);
    }
  }
